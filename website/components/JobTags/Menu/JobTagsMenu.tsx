...
export const JobTagsMenu = (props: ITagsMenu) => {
    ...
    const {loading, selectedTags, tagsToSelect, selectTag, removeTag, inputVal, setInputVal, canCreateNew, onSubmit, error
      ,editTag,
      setEditTag,
      isEditModalOpen,
    } = useTagsMenu(123, onSetSelectedTags, onSetTagsToSelect, tagsSelected, selectableTags);
    ...
    useEffect(() => {
      const handleClickOutside = (event: MouseEvent) => {
        if (ref.current && !ref.current.contains(event.target as Node) && !isEditModalOpen) {
          onOutsideClick();
        }
      };
      ...
      <ChipsWrapper>
            {
              selectedTags.map(tag=>(
                <StyledChip key={tag.label} bgcolor={tag.color} label={renderInnerChipContent(
                  {
                    tag: tag,
                    onSetEditTag: setEditTag
                  }
                )} deleteIcon={<ClearIcon />} onDelete={()=>{removeTag(tag.label)}}/>
              ))
            }
          </ChipsWrapper>
          ...
          <StyledMenuList>
            {
              tagsToSelect.map(tag=>(
                ...
                <StyledListItemText bgcolor={tag.color} onClick={()=>{selectTag(tag.label)}}>{tag.label}</StyledListItemText>
                <ListItemIcon onClick={()=>{selectTag(tag.label)}}>
                  <StyledCheckIcon  bgcolor={tag.color}/>
                </ListItemIcon>
    ...
    return (
      <>
        {
          editTag ? <EditTagModal tag={editTag} isOpen={isEditModalOpen} onClose={()=>{
...
const StyledCheckIcon = styled(CheckIcon)<IChip>`
